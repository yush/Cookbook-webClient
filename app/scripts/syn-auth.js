angular.module('syn-auth', ['em-utils'], function($provide) {
	...
	$provide.factory('synConn', function(SynAuth, emAuth, $http, $q) {
		var synConn = {
			opts: {
		...
		function getUrlSign(uri, done) {
            getConnectionFromURi(uri, function(err, conn){
                url = conn.SessionSign(uri);
                return done(err, url);
            });				
		}
